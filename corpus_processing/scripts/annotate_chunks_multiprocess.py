...
def process_single_file(filepath):
    ...
    try:
        ...
        for doc in nlp.pipe(lines, disable=disabled, batch_size=batch_size):
            all_tokens.extend([tok.text for tok in doc])
            all_lemmas.extend([tok.lemma_ for tok in doc])
            all_pos.extend([tok.pos_ for tok in doc])
...
def process_corpus(args):
    ...
    # Show some already processed files if not too many
    if already_processed and len(already_processed) <= 10 and args.verbose:
        print("\nSkipping existing files:")
        for fp in already_processed[:10]:
            print(f"  - {fp.name}")
    ...
    with mp.Pool(
        ...
        for filepath, data, error in pool.imap_unordered(
            ...
            if error:
                ...
                output_path = args.output_dir / f"{filepath.stem}.csv"
                annotation_df.to_csv(output_path, index=False)
                processed += 1
                
                if args.verbose and processed <= 5:
                    print(f"  Processed: {filepath.name} ({len(data['Token'])} tokens)")
